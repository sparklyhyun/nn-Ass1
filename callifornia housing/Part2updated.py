...
def main():
    ...
    #Compare Min CV Errors across Learning Rates
    cv_err_min=np.min(cv_err, axis=1)

    plt.figure(2)
    plt.plot(learning_rate, cv_err_min)
    plt.xlabel('Learning Rate')
    plt.ylabel('Min Cross-Validation Error')
    plt.show()


    #The optimal learning rate is 0.5* 10**-6

    #Define optimal learning rate as one that gives the lowest CV error?
    bestlr=learning_rate[np.argmin(np.min(cv_err, axis=1))]
    print('learning rate that gives lowest error: {}'.format(bestlr))

    #Split the training and test set: Question: Will they be split equally as above?
    trainXV, testXV, trainYV, testYV = model_selection.train_test_split(X_data, Y_data, test_size=0.3, random_state=42)
    ...
    with tf.Session() as sess:
        ...
        for i in range(epochs):
            ...
            batch_test_err = []
            for start, end in zip(range(0, N, batch_size), range(batch_size, N, batch_size)):
                train_op.run(feed_dict={x: stdtestXV[start:end], y_: testYV[start:end]}) #do I train with the test set again? Since need to use best lr

                #Tests error for 1 batch
                batch_test_err.append(loss.eval(feed_dict={x: stdtestXV, y_:testYV}))

            #Test error for eppochs: Mean batch error
            test_err.append(sum(batch_test_err)/len(batch_test_err))

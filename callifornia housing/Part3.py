...
epochs = 10
batch_size = 32
learning_rate = 0.5*10**-6 #Optimal learning rate from part 2
beta=10**-3
seed = 10
np.random.seed(seed) # 5 fold cross validation
no_folds=5

def train(num_neuron):
...
def main():
    ...
    for i in range(len(num_neuron)):
        plt.plot(range(epochs), cv_err[i], label= 'Number of Hidden Neurons= {}'.format(num_neuron[i]))
        min_cost.append((min(cv_err[i])))
    ...
    plt.figure(2)
    plt.plot(num_neuron, min_cost)
    plt.xlabel('Number of Neurons')
    plt.ylabel('Min CV Error Across Iterations') # the mean square error is high because it is a squared error.
    # also the values for y is very high
    plt.title('GD Learning')
    plt.show()

    bestlr=num_neuron[np.argmin(np.min(cv_err, axis=1))] # get the neuron which gives the minimum lowest error
    print('Number of Hidden Neurons that gives lowest error: {}'.format(bestlr))

    idx=num_neuron.index(bestlr)

    plt.figure(3)
    plt.plot(range(epochs), cv_err[idx], label= 'Number of Hidden Neurons= {}'.format(num_neuron[idx]))
    plt.xlabel('Number of Epochs')
    plt.ylabel('Mean CV Error Across Folds') # the mean square error is high because it is a squared error.
    # also the values for y is very high
    plt.title('GD Learning')
